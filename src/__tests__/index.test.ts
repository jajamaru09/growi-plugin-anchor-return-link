...
describe('rehypeAnchorReturnLink', () => {
  ...
  it('accepts an injected log function and calls it at least once', () => {
    const calls: unknown[][] = [];
    const log = (...args: unknown[]) => { calls.push(args); };
    const input = `<p><a href="#section" ${ua}>go</a></p><h2 id="section">Section</h2>`;
    unified()
      .use(rehypeParse, { fragment: true })
      .use(rehypeAnchorReturnLink, { log, context: 'test' })
      .use(rehypeStringify)
      .processSync(input);
    expect(calls.length).toBeGreaterThan(0);
    const flat = calls.map(a => a.map(String).join(' ')).join('\n');
    expect(flat).toContain('pass1');
    expect(flat).toContain('pass3');
  });
});
